...
import { AddEmployeeCertificates } from './AddEmployeeCertificates';
import { ListEmployeeCertificates } from './ListEmployeeCertificates';
...
export function EmployeeCertificates() {
    ...
    console.log(" before feched ",complianceData);
      ...
      const filteredData = complianceData.filter((doc) => {
       return doc.employeeName.toLowerCase().includes(searchTerm.toLowerCase());
      });
...
>
        <AddEmployeeCertificates onCancel={handleCancelAddDocument}/>

</motion.div>
    ):(
        <ListEmployeeCertificates filteredData={filteredData}/>
