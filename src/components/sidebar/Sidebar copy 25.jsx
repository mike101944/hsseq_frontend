...
export const Sidebar = forwardRef(({
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      // Tunataka kuseti collapsed tu kwenye tablet ili kuruhusu desktop ifunguke kwa default
      if (tablet) setSidebarCollapsed(true);
      // Kwa desktop, tuifungue (ikiwa haijafungwa kwa click)
  ...
  // ===============================
  // KAZI ILIYOREKEBISHWA HAPA 👇
  // ===============================
  const getSidebarClasses = () => {
    const base = `
      h-full bg-white
      border-r border-green-300 flex flex-col p-4 pl-0 pr-0 transition-all duration-300
    `; // 🔥 Duration imepunguzwa, overflow-x-hidden IMEONDOLEWA
    
    if (isMobile)
      return `${base} fixed top-0 left-0 w-64 overflow-y-auto z-50 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`;
    
    // 🔥 MUHIMU: Ikiwa Collapsed (Tablet au Desktop), tumia overflow-visible
    if (sidebarCollapsed) {
      return `${base} relative z-40 w-20 overflow-visible`;
    }

    // Ikiwa Expanded (Desktop)
    return `${base} relative z-40 w-64 overflow-y-auto`;
  };
  ...
  const renderModuleItem = (module) => {
    ...
    if (sidebarCollapsed && !isMobile) {
      return (
        // 🔥 MUHIMU: Ongeza 'relative z-50' hapa
        <li key={module.id} className="relative z-50"> 
          <div
            className="relative"
          ...
          >
            ...
            {hoveredModule === module.id && (
              <div
                // 🔥 Hii inapaswa kuwa ya juu zaidi
      ...
      >
        {isMobile && (
          <div className="flex justify-end mb-4">
            <button
              className="p-1 rounded-md hover:bg-gray-100 transition" // Imebadilishwa to gray-100 na text-gray-600 kwa white sidebar
              onClick={() => setSidebarOpen(false)}
            >
        ...
        {!isMobile && (
          <div className="absolute top-4 -right-3 z-50">
            <button
              ...
              className="w-6 h-6 bg-green-600 border-2 border-white rounded-full flex items-center justify-center hover:bg-green-700 shadow-md"
            >
              {sidebarCollapsed ? (
        ...
        {/* 🔥 MUHIMU: Nav tag sasa ina `overflow-y-auto` tu. Scrollbar custom styles zimebaki. */}
        <nav 
        className={`flex-1 ${!sidebarCollapsed || isMobile ? "overflow-y-auto" : ""}`}
        style={{
          scrollbarWidth: "none",
