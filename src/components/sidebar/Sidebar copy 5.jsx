...
export const Sidebar = forwardRef(({ 
  ...
  const getSidebarClasses = () => {
    const baseClasses = `
      ...
      shadow-2xl shadow-green-900/50 flex flex-col p-4 transition-all duration-300 z-20
      overflow-x-hidden /* PREVENT HORIZONTAL SCROLLING */
    `;

    if (isMobile) {
      return `
        ${baseClasses} fixed top-0 left-0 w-64 overflow-y-auto
      ...
      return (
        <li key={module.id}>
          ...
          >
            ...
            <div className="absolute left-full top-1/2 transform -translate-y-1/2 ml-2 px-2 py-1 bg-gray-900 text-white text-sm rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-50 pointer-events-none">
              {module.name}
            </div>
          </button>
        </li>
      ...
      <li key={module.id}>
        <div className="space-y-1">
          ...
          >
            ...
            {/* Tooltip for single items in collapsed state */}
  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation modules */}
        <nav className="flex-1 min-w-0"> {/* ADDED min-w-0 to prevent overflow */}
          <ul className="space-y-2 min-w-0"> {/* ADDED min-w-0 to prevent overflow */}
        ...
        {(!sidebarCollapsed || isMobile) && (
          <div className="mt-auto pt-4 border-t border-green-600 min-w-0"> {/* ADDED min-w-0 */}
