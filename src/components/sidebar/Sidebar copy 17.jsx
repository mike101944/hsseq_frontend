...
export const Sidebar = forwardRef(({
  ...
  const getSidebarClasses = () => {
    const base = `
      ...
      shadow-2xl shadow-green-900/50 flex flex-col p-4 transition-all duration-300 overflow-x-hidden
    `;
    if (isMobile)
      return `${base} fixed top-0 left-0 w-64 overflow-y-auto z-50 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`;
    if (isTablet)
      return `${base} relative z-40 ${sidebarCollapsed ? "w-20 overflow-y-auto" : "w-64 overflow-y-auto"}`;
    return `${base} relative z-40 ${sidebarCollapsed ? "w-20 overflow-y-auto" : "w-72 overflow-y-auto"}`;
  };
    ...
    return (
      <li key={module.id}>
        <div className="space-y-1">
          <button
            onClick={() =>
              hasSubItems ? toggleDropdown(module.id) : handleNavigation(module.path)
            }
              ...
              {module.subItems.map((sub) => {
                ...
                return (
                  <li key={sub.id}>
                    ...
                    >
                      <div className={`w-1 h-1 rounded-full ${active ? "bg-white" : "bg-green-400"}`} />
                      <span>{sub.name}</span>
                    </button>
                  </li>
                );
  ...
  return (
    <>
      ...
      >
        ...
        {/* LOGO - Fixed at top */}
        <div className={`flex items-center gap-3 mb-8 px-2 flex-shrink-0 ${sidebarCollapsed && !isMobile ? "justify-center" : ""}`}>
        ...
        <div className="mt-auto pt-4 border-t border-green-600 flex-shrink-0">
          {sidebarCollapsed && !isMobile ? (
